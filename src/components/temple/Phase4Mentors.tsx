...
function generateAdvice(mentor: (typeof MENTORS)[0], concern: string): string {
    const templates: Record<number, string[]> = {
        1: [`"${concern}"에 대해 먼저 물어봐야 합니다 — 당신은 정말 이것을 원하는가, 아니면 두려움으로부터 도망치는 것인가? 참된 자아를 탐구하면 답을 찾을 것입니다.`, `내가 무소부지(無所不知)라 생각했지만, 결국 내가 아는 것은 아무것도 모른다는 것이었습니다. "${concern}"도 마찬가지 - 먼저 겸손히 배우려는 자세가 필요합니다.`],
        2: [`"${concern}"의 이상적인 모습을 먼저 그려보세요. 이데아의 세계에서 완벽한 해답이 이미 존재합니다. 그 진리를 향해 한 걸음씩 나아가세요.`, `현실은 이상적 형태의 그림자일 뿐입니다. "${concern}"에서 진정한 선(善)이 무엇인지 바라보면 길이 보입니다.`],
        3: [`"${concern}"은 한 번의 큰 결정이 아니라 매일의 작은 습관으로 해결됩니다. 탁월함은 반복에서 나옵니다.`, `중용(中庸)이 답입니다. "${concern}"에서 극단을 피하고 균형 잡힌 접근으로 덕(德)을 실천하세요.`],
        4: [`사랑이 모든 것의 답입니다. "${concern}"을 두려움이 아닌 사랑의 관점으로 바라보면 어떨까요? 가장 가까운 사람에게 작은 선행을 베풀어보세요.`, `"${concern}"에 지쳐있다면, 잠시 멈추고 다른 누군가를 도와보세요. 봉사 속에서 자신을 잃고, 자신을 찾습니다.`],
        5: [`"${concern}"에 대한 집착을 잠시 내려놓아 보세요. 무소유의 마음으로 바라보면 진정 필요한 것만 남습니다.`, `지금 이 순간, 당신에게 진정으로 필요한 것은 무엇인가요? "${concern}"에서 불필요한 것들을 덜어내면 본질이 드러납니다.`],
        6: [`지금 이 순간, "${concern}"은 생각 속에만 존재합니다. 현재로 돌아오세요. 진정한 힘은 지금 여기에 있습니다.`, `"${concern}"에 대한 생각을 관찰하되, 그 생각과 동일시하지 마세요. 당신은 생각이 아니라, 생각을 알아차리는 존재입니다.`],
        7: [`당신의 잠재의식은 "${concern}"에 대한 답을 이미 알고 있습니다. 그것을 현실로 끌어당기기 위해 생생하게 이미 이루어진 것처럼 느껴보세요!`, `끌어당김의 법칙은 흔들리지 않습니다. "${concern}"에서 원하는 결과를 명확히 그리고, 그 감정을 먼저 느끼세요. 우주가 반응할 것입니다.`],
        8: [`"${concern}"의 해결된 모습을 지금 당장 상상하세요. 감사함과 기쁨을 느끼면서요! 시크릿이 작동합니다.`, `원하는 것에만 집중하세요. "${concern}"에서 두려워하는 것이 아닌 원하는 것을 바라보면, 그것이 현실이 됩니다.`],
        9: [`성공이란 가치 있는 목표를 향해 점진적으로 나아가는 것입니다. "${concern}"에서 오늘 당장 할 수 있는 한 가지를 찾아 행동하세요.`, `당신은 생각하는 것이 됩니다. "${concern}"에 대해 성공한 자신을 매일 상상하세요. 가장 이상한 비밀이 여기 있습니다.`],
        10: [`잠재의식에 "${concern}"의 해결책을 심어놓으세요. 자기 전 긍정 확언을 반복하면, 수면 중 잠재의식이 그것을 현실로 만들어갑니다.`, `당신의 잠재의식은 24시간 작동합니다. "${concern}"에 대한 긍정적 이미지를 명확히 심어두면, 잠재의식이 알아서 길을 찾을 것입니다.`],
    };
...
function generateScenario(type: string, concern: string): { timeline: string; metaphor: string; description: string } {
    const scenarios: Record<string, { timeline: string; metaphor: string; description: string }> = {
        "현상 유지": {
            timeline: "6개월 ~ 1년",
            metaphor: "🌊 잔잔한 강물처럼",
            description: `지금의 방식을 유지한다면, "${concern}"은 시간이 지남에 따라 서서히 안정화될 것입니다. 큰 변화 없이 현재의 상황이 지속되며, 안전하지만 성장의 속도는 더딜 수 있습니다.`,
        },
        "과감한 혁신": {
            timeline: "3~6개월",
            metaphor: "🚀 로켓처럼 도약하다",
            description: `두려움을 밀어내고 과감히 행동한다면, "${concern}"은 3개월 내 극적인 전환점을 맞이할 것입니다. 처음엔 혼란스럽지만, 6개월 후엔 전혀 다른 차원의 삶이 펼쳐집니다.`,
        },
        "내면 성찰": {
            timeline: "1~3개월",
            metaphor: "🌸 꽃망울이 피어나듯",
            description: `조용히 내면으로 돌아와 "${concern}"의 근원을 바라보면, 1개월 안에 명확한 깨달음이 찾아옵니다. 서두르지 말고 자신과의 대화를 통해 진정한 해답을 발견하세요.`,
        },
    };
